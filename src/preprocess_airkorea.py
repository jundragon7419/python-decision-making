...
INTERMEDIATE_PATH = "data/processed/airkorea_daily_nojoin.parquet"
OUTPUT_PATH = "data/processed/airkorea_daily.parquet"
...
def aggregate_daily(df: pd.DataFrame) -> pd.DataFrame:
    ...
    결측치는 mean 계산에서 자동 제외되며, 오염물질별 유효 관측 시간이
    MIN_HOURS_PER_DAY 미만인 날은 해당 오염물질 일평균을 NaN 처리한다.
    """
    ...
    grouped.loc[grouped["pm10_n"] < MIN_HOURS_PER_DAY, "pm10"] = pd.NA
    grouped.loc[grouped["pm25_n"] < MIN_HOURS_PER_DAY, "pm25"] = pd.NA
    return grouped.drop(columns=["pm10_n", "pm25_n"])
